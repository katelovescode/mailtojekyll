...
describe JekyllEmail do
  ...
  device_titles = {
    'gmail_desktop/' => ' from Gmail Desktop',
    'droid_gmail/' => ' from Android Gmail',
    'ios_gmail/' => ' from iOS Gmail', 
    'ios_mail/' => ' from iOS Mail', 
    'lg_mail/' => ' from LG Mail',
    'outlook/' => ' from Outlook',
    'owa/' => ' from OWA'
    }
    
  device_titles.each do |key,val|
    let(:device) { key }
    let(:identification) { val }
  
    context "given an email sent#{val}" do
